...
async def generate_ratings_stage(
    db: AsyncSession, question_id: int, answer_id: int
) -> int:
    try:
    ...
    async with async_session() as db:
        while True:
            ...
            for paragraph in paragraphs:
                ...
                tasks.append(task)
            questions_by_paragraph = await asyncio.gather(*tasks)
            ...
            for question_id in question_ids:
                task = asyncio.create_task(generate_answers_stage(db, question_id))
                tasks.append(task)
            answers_by_question = await asyncio.gather(*tasks)
...
async def start_background_process_s2s():
    try:
        await process_all_paragraphs_s2s(batch_size=5)
    except Exception as e:
